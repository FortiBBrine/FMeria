...
public class CommandHP implements CommandExecutor {
	...
	public CommandHP(FMeria plugin) {
		this.plugin = plugin;
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (plugin.getConfig().getBoolean(faction+".hospital", false)==false) {
			p.sendMessage(messageConfig.getString("message.nonHospitalFaction"));
			return true;
		}
		
		int rank = plugin.getConfig().getInt(faction+".users."+p.getName());
		int needRank = plugin.getConfig().getInt(faction+".hospitalRank");
		if (rank < needRank) {
			p.sendMessage(messageConfig.getString("message.nonRank"));
			return true;
		}
		if (p.getName().toLowerCase().equals(args[0].toLowerCase())) {
			p.sendMessage(messageConfig.getString("message.errorPlayer"));
			return true;
		}
		...
		if (!((int) p.getLocation().distance(player.getLocation()) < 5)) {
